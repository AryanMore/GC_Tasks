...
def add_products_bulk(products: list[Product]):
    try:
        collection.insert_many([p.dict() for p in products], ordered=False)
...
def search_products(
    ...
    return [product_helper(p) for p in collection.find(query)]
...
# ---------------- DELETE ----------------
@router.delete("/delete-by-filter")
